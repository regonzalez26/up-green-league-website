...
class ArticleFeatureBar extends React.Component {
  constructor(props){
    super(props)
    this.state = {featured_article_ids: []}
  }
  ...
  componentDidMount = () => {
    if(this.props.featured_article_ids){
      this.setState({featured_article_ids: this.props.featured_article_ids})
    }
  ...
  render() {
    return(
      <div id="article_feature_bar_container">
        ...
        {
          this.state.featured_article_ids.map((id, idx) =>(
